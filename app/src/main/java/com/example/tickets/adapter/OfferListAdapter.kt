...
import com.example.tickets.adapter.utils.OfferListDiffUtils
...
class OfferListAdapter : RecyclerView.Adapter<OfferListAdapter.ViewHolder>() {
    ...
    fun setItems(offerList: List<Offer>) {
        val diffResult = DiffUtil.calculateDiff(OfferListDiffUtils(items, offerList))
        items.clear()
    ...
    fun sortByPrice() {
        setItems(items.sortedBy { it.price })
    }

    fun sortByDuration() {
        setItems(items.sortedBy { it.flight.duration })
    }
